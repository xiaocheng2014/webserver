...
class Redis_Test extends TestSuite
{
    ...
    public function testKeys()
    {
        ...
        $this->assertEquals([], $this->redis->keys(RedisTest . phprand() . rand() .rand().'*'));
    }
    ...
    private function startSessionProcess($sessionId, $sleepTime, $background, $maxExecutionTime = 300, $locking_enabled = true, $lock_wait_time = null, $lock_retries = -1, $lock_expires = 0, $sessionData = '', $sessionLifetime = 1440)
    {
        if (substr(php_uname(), 0, 7) == "Windows"){
            ...
            $command = RedisTest . phpself::getPhpCommand('startSession.php') . implode(' ', $commandParameters);
    ...
    {
        $command = RedisTest . phpself::getPhpCommand('getSessionData.php') . escapeshellarg($this->getFullHostPath()) . ' ' . $this->sessionSaveHandler . ' ' . escapeshellarg($sessionId) . ' ' . escapeshellarg($sessionLifetime);
        exec($command, $output);
	...
	$args = array_map('escapeshellarg', [$sessionId, $locking, $destroyPrevious, $sessionProxy]);
        ...
        $command = RedisTest . phpself::getPhpCommand('regenerateSessionId.php') . escapeshellarg($this->getFullHostPath()) . ' ' . $this->sessionSaveHandler . ' ' . implode(' ', $args);
